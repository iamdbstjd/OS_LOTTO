...
본 보고서는 Django와 Docker를 활용하여 개발한 `Lotto 사이트`의 설계, 구현 과정, 주요 화면, 테스트 결과를 정리한 문서이다. 이 사이트는 6/45 로또 서비스를 웹으로 구현한 과제용 프로젝트이며, 일반 사용자는 회원가입과 로그인 후 수동 또는 자동으로 로또를 구매할 수 있다. 구매한 티켓은 사용자별로 저장되며, 추첨이 완료되면 사용자는 본인의 당첨 여부와 당첨금을 확인할 수 있다. 관리자는 판매 현황과 판매 내역을 확인하고, 추첨 기능을 실행하며, 회차별 당첨자 수와 이익을 확인할 수 있다.

프로젝트 작성 과정에서는 GPT(ChatGPT/OpenAI Codex)를 사용했다. GPT는 요구사항 정리, Django 구조 설계, Docker 구성, 화면 설명 작성, 테스트 코드 작성, 보고서 문장 정리 과정에서 보조 도구로 활용했다.
